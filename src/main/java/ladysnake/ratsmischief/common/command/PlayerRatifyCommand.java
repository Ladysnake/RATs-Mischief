...
public class PlayerRatifyCommand {
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(CommandManager.literal("ratify")
    ...
    private static int execute(ServerCommandSource source, Collection<? extends PlayerEntity> targets) {
        if (FabricLoader.getInstance().isModLoaded("requiem")) {
            ...
            if (targets.size() == 1) {
                source.sendFeedback(Text.translatable("commands.ratify.success.single", (targets.iterator().next()).getDisplayName()), true);
            } else {
                source.sendFeedback(Text.translatable("commands.ratify.success.multiple", targets.size()), true);
            }
        } else {
            source.sendError(Text.translatable("commands.ratify.error.requiresRequiem"));
        }
