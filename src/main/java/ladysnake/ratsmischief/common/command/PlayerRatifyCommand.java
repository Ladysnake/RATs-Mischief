...
public class PlayerRatifyCommand {
    ...
    private static int execute(ServerCommandSource source, Collection<? extends PlayerEntity> targets) {
        if (FabricLoader.getInstance().isModLoaded("requiem")) {
            ...
            if (targets.size() == 1) {
                source.sendFeedback(new TranslatableText("commands.ratify.success.single", (targets.iterator().next()).getDisplayName()), true);
            } else {
                source.sendFeedback(new TranslatableText("commands.ratify.success.multiple", targets.size()), true);
            }
        } else {
