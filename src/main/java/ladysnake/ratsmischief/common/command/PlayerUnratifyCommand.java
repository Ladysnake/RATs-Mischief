...
public class PlayerUnratifyCommand {
    ...
    private static int execute(ServerCommandSource source, Collection<? extends PlayerEntity> targets) {
        if (FabricLoader.getInstance().isModLoaded("requiem")) {
            ...
            if (targets.size() == 1) {
                source.sendFeedback(Text.translatable("commands.unratify.success.single", (targets.iterator().next()).getDisplayName()), true);
            } else {
                source.sendFeedback(Text.translatable("commands.unratify.success.multiple", targets.size()), true);
            }
        } else {
            source.sendError(Text.translatable("commands.unratify.error.requiresRequiem"));
        }
